...
hdrTextureLoader.load('/textures/workshop.hdr', function (texture) {
    ...
    for (let i = 0; i < vertices.length; i++) {
        ...
        sphere.position.set(vertices[i][0] * sphereRadius, vertices[i][1] * sphereRadius, vertices[i][2] * sphereRadius);
        spheres.push(sphere)
        rotationObject.add(sphere);

        const planeGeometry = new THREE.PlaneGeometry(Math.sqrt(0.05), Math.sqrt(0.05));
...
const tick = () =>
{
    ...
    for (let i = 0; i < spheres.length; i++) {
        ...
        let direction = new THREE.Vector3().subVectors(camera.position, sphere.position);
